...
import { Bell, Menu, X, Send, ShoppingBag, User, LogOut, ChevronDown } from "lucide-react";
                ...
                >
                  ...
                  >
                    <div className="flex w-full items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-[var(--color-ink)]">{a.titulo}</span>
                    </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <div className="px-2 py-1.5">
                <div className="text-sm font-bold text-[var(--color-ink)]">{currentUser.name}</div>
                <div className="text-xs text-muted-foreground">{currentUser.handle}</div>
              </div>
              <DropdownMenuSeparator />
              <DropdownMenuItem asChild className="focus:bg-surface-2! focus:text-inherit!">
                <Link to="/publish" className="flex items-center gap-2">
                  <Send className="h-4 w-4 text-muted-foreground" /> Publicar pasaje
                </Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild className="focus:bg-surface-2! focus:text-inherit!">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem
                onClick={() => toast("Cerrar sesión no está disponible en este prototipo")}
        ...
        <div className="border-t border-[var(--color-ink)]/12 bg-surface-2 md:hidden">
          <div className="mx-auto flex max-w-7xl flex-col gap-1 px-4 py-3">
            ...
            <button
              onClick={() => {
                setOpen(false);
                toast("Cerrar sesión no está disponible en este prototipo");
              }}
